...
class DataValidator:
    ...
    @classmethod
    def validate_questions(cls, questions: List[Question]) -> Tuple[List[Question], List[Dict[str, Any]]]:
        """
        ...
        Returns:
            Tuple of (valid_questions, validation_error_details)
        """
        valid_questions = []
        ...
        for i, question in enumerate(questions):
            ...
            if errors:
                error_detail = {
                    'message': f"Question {i+1} (ID: {question.id}): {'; '.join(errors)}",
                    'question_id': question.id,
                    'question_index': i+1,
                    'raw_content': question.rawContent,
                    'errors': errors
                }
                validation_error_details.append(error_detail)
            else:
                valid_questions.append(question)

        return valid_questions, validation_error_details
    ...
    def validate_all_data(
        ...
        # Validate questions
        valid_questions, question_error_details = cls.validate_questions(questions)
        validation_results['valid_questions'] = valid_questions
        validation_results['errors']['question_errors'] = question_error_details
