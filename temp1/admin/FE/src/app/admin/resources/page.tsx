...
export default function ResourceAccessPage() {
  ...
  /**
   * Fetch resource access data từ API
   */
  const fetchResourceAccess = async () => {
    try {
      ...
      const [logsResponse, statsResponse] = await Promise.all([
        fetch(`/api/admin/resource-access?${queryParams}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }),
        fetch("/api/admin/resource-access/stats", {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }),
      ]);

      if (!logsResponse.ok || !statsResponse.ok) {
        throw new Error("Failed to fetch resource access data");
      }
      ...
      const mockStats: ResourceAccessStats = {
        ...
        averageRiskScore: 15,
        highRiskAttempts: 23,
        accessByResourceType: { VIDEO: 456, PDF: 234, COURSE: 189, LESSON: 156, EXAM: 89 },
        accessByAction: { VIEW: 567, DOWNLOAD: 234, STREAM: 189, START_EXAM: 89 },
        topResources: [
