...
export default function LaTeXRenderer({
  ...
  className = '',
  showSolution = true
}: LaTeXRendererProps): JSX.Element {
  ...
  const extractData = () => {
    try {
      ...
      if (!content.includes('\\begin{ex}') || !content.includes('\\end{ex}')) {
        ...
        logger.info('Hiển thị nội dung đơn giản không phải LaTeX:', simpleData);
        return;
      }
      ...
      if (loigiaiMatch && loigiaiMatch.index !== undefined) {
        try {
          ...
          logger.error('Lỗi khi trích xuất lời giải với phương pháp cân bằng dấu ngoặc:', error);

          // Fallback: Sử dụng regex đơn giản nếu phương pháp cân bằng thất bại
          const solutionMatch = content.match(/\\loigiai\s*\{([\s\S]*?)\}/);
          if (solutionMatch) {
            solution = solutionMatch[1].trim();
            logger.warn('Đã sử dụng regex đơn giản để trích xuất lời giải, có thể không chính xác với dấu ngoặc lồng nhau');
          }
        }
      }
      ...
      logger.info('Dữ liệu trích xuất từ LaTeX (đã xử lý):', extractedData);

      // Đã xác định loại câu hỏi và xử lý tương ứng
    } catch (err) {
      logger.error('Lỗi khi trích xuất dữ liệu:', err);
      ...
      try {
        ...
        if (data) {
          ...
          const processedData = {
            ...
            answers: Array.isArray(data.answers) ? data.answers.map((answer: unknown, index: number) => {
              ...
              let content = answerObj.content || answerObj.text || '';
          ...
          logger.info('Dữ liệu trích xuất từ extractFromLatex:', processedData);
        ...
        logger.error('Lỗi khi sử dụng extractFromLatex:', extractError);
      }
      ...
      logger.info('Sử dụng dữ liệu dự phòng khi gặp lỗi:', fallbackData);
    }
  };
  ...
  const renderMathWithKaTeX = (text: string) => {
    ...
    const renderFormula = (formula: string, isDisplayMode = false) => {
      try {
        ...
        logger.error('Lỗi khi render công thức toán học:', error);
        logger.error('Công thức gây lỗi:', formula);
      ...
      <div className="question-container">
        ...
        {(extractedData.type === 'multiple-choice' || extractedData.type === 'true-false') && (
          <div className="answers-container">
            {answers.slice(0, 4).map((answer: unknown, index: number) => {
              ...
              if (extractedData.correctAnswer) {
                if (Array.isArray(extractedData.correctAnswer)) {
                  ...
                  isCorrect = extractedData.correctAnswer.includes(answerObj.id);
                } else if (typeof extractedData.correctAnswer === 'string') {
                  // Nếu correctAnswer là string, so sánh trực tiếp với answer.id
                  isCorrect = extractedData.correctAnswer === answerObj.id;
              ...
              return (
                <AnswerBox
                  key={answerObj.id || index}
                  answerClass={`${answerClass} ${isCorrect ? 'correct' : ''}`}
                  iconStyle={iconStyle}
                  label={label}
                  content={processAnswerContent(answerObj.content || answerObj.text || '')}
