...
export const mockQuestions: AdminQuestion[] = [
  ...
  },
  {
    ...
    category: 'Toán học',
    tag: ['Logarithm', 'Phương trình', 'Lớp 12'],
  ...
  // Hóa học - Lớp 10
  {
    ...
    difficulty: QuestionDifficulty.EASY,
    category: 'Hóa học',
    tag: ['Bảng tuần hoàn', 'Nguyên tố', 'Lớp 10'],
  ...
  // Tiếng Anh - Lớp 9
  {
    ...
    difficulty: QuestionDifficulty.MEDIUM,
    category: 'Tiếng Anh',
    tag: ['Grammar', 'Present Perfect Continuous', 'Lớp 9'],
  ...
  // True/False question
  {
    ...
    difficulty: QuestionDifficulty.EASY,
    category: 'Toán học',
    tag: ['Hình học', 'Định lý Pythagoras', 'Lớp 8'],
...
export function getQuestionsByDifficulty(difficulty: QuestionDifficulty): AdminQuestion[] {
...
export function searchQuestions(query: string): AdminQuestion[] {
  ...
  return mockQuestions.filter(question =>
    ...
    question.tag.some((tag: string) => tag.toLowerCase().includes(searchTerm)) ||
