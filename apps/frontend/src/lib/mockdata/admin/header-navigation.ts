 ...
 * Mock data cho current admin user trong header
 */
export const mockAdminHeaderUser: AdminUser = {
  id: 'admin-001',
  name: 'Admin User',
...
export const adminHeaderMockService = {
  ...
  getCurrentUser: (): Promise<AdminUser> => {
    return MockDataUtils.simulateApiCall(mockAdminHeaderUser, 200);
  },
  ...
  updateUserProfile: (updates: Partial<AdminUser>): Promise<AdminUser> => {
    const updatedUser = { ...mockAdminHeaderUser, ...updates };
