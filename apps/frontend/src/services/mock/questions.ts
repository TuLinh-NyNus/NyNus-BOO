...
import { ensureSingle } from '@/lib/utils/filter-type-adapters';
...
export class MockQuestionsService {
  ...
  static async listQuestions(params: {
    ...
    if (keyword) {
      ...
      filteredQuestions = filteredQuestions.filter(q =>
        ...
        q.tag.some((tag: string) => tag.toLowerCase().includes(searchTerm))
      );
    }
  ...
  static async saveQuestion(question: Question): Promise<{ ok: true }> {
    ...
    try {
      ...
      const existingIndex = savedData.questions.findIndex((q: Question) => q.id === question.id);
      if (existingIndex >= 0) {
