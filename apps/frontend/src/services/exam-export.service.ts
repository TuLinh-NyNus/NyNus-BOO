...
// Type for answer content that can be string or object
...
export class ExamExportService {
  // Helper function to extract answer content
  private static getAnswerContent(answer: unknown): string {
    if (typeof answer === 'string') return answer;
    const answerObj = answer as AnswerContent;
    return answerObj?.content || answerObj?.text || '';
  }
  ...
  private static async exportToPDF(
    ...
    addText(`Thời gian: ${exam.durationMinutes} phút`, 12);
    ...
    questions.forEach((question, index) => {
      ...
      // Question options
      if (question.type === QuestionType.MULTIPLE_CHOICE && question.answers) {
        question.answers.forEach((answer, answerIndex) => {
          const letter = String.fromCharCode(65 + answerIndex); // A, B, C, D
          const content = this.getAnswerContent(answer);
          addText(`${letter}. ${content}`, 11);
        });
      }

      // Show correct answer if requested
      if (options.includeAnswers && question.correctAnswer) {
        addText(`Đáp án: ${question.correctAnswer}`, 10, true);
      }
  ...
  private static async exportToExcel(
    ...
    const examInfo = [
      ...
      ['Ngày tạo', exam.createdAt ? new Date(exam.createdAt).toLocaleDateString('vi-VN') : ''],
    ];
    ...
    const questionData = questions.map((question, index) => [
      ...
      question.points || 0,
      this.getAnswerContent(question.answers?.[0]) || '',
      this.getAnswerContent(question.answers?.[1]) || '',
      this.getAnswerContent(question.answers?.[2]) || '',
      this.getAnswerContent(question.answers?.[3]) || '',
      options.includeAnswers ? question.correctAnswer || '' : '',
    ...
    let html = `
      ...
      <body>
        <div class="header">
          ...
          <div class="info">Thời gian: ${exam.durationMinutes} phút</div>
    ...
    questions.forEach((question, index) => {
      ...
      if (question.type === QuestionType.MULTIPLE_CHOICE && question.answers) {
        ...
        question.answers.forEach((answer, answerIndex) => {
          ...
          const content = this.getAnswerContent(answer);
          html += `<div class="option">${letter}. ${content}</div>`;
        });
        html += '</div>';
      }

      if (options.includeAnswers && question.correctAnswer) {
        html += `<div class="answer">Đáp án: ${question.correctAnswer}</div>`;
      }
  ...
  private static async exportToCSV(
    ...
    const rows = questions.map((question, index) => [
      index + 1,
      this.escapeCSV(question.content),
      question.type,
      question.difficulty || 'MEDIUM',
      question.points || 0,
      this.escapeCSV(this.getAnswerContent(question.answers?.[0])),
      this.escapeCSV(this.getAnswerContent(question.answers?.[1])),
      this.escapeCSV(this.getAnswerContent(question.answers?.[2])),
      this.escapeCSV(this.getAnswerContent(question.answers?.[3])),
      ...(options.includeAnswers ? [this.escapeCSV(typeof question.correctAnswer === 'string' ? question.correctAnswer : JSON.stringify(question.correctAnswer) || '')] : [])
    ]);
  ...
  private static async exportMultipleToExcel(
    ...
    exams.forEach((item, index) => {
      const _sheetName = `Đề ${index + 1}`;
      const _result = this.exportToExcel(item.exam, item.questions, options);
  ...
  private static async exportMultipleToCSV(
    ...
    exams.forEach(item => {
      item.questions.forEach((question, questionIndex) => {
        rows.push([
          this.escapeCSV(item.exam.title),
          this.escapeCSV(item.exam.subject),
  ...
  private static async exportMultipleAsZip(
    _exams: Array<{ exam: Exam; questions: Question[] }>,
  ...
  private static calculateExamStatistics(exam: Exam, questions: Question[]) {
    ...
    return {
      ...
      averageTimePerQuestion: (exam.durationMinutes / questions.length).toFixed(1),
